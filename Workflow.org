...
#+PROPERTY: header-args:emacs-lisp :tangle ./lisp/dn-workflow.el
...
This Org file produces a file called =dn-workflow.el= which gets loaded up in =init.el=; export it as a feature so that it can be loaded with =require=.
